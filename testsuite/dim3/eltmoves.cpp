...
class ElementaryMovesTest : public CppUnit::TestFixture {
    ...
    public:
        ...
        void zeroTwoMoves() {
            {
                // Test a 0-2 move where both faces on either side of the
                // edge are glued to something.
                /**
                * 3-dimensional triangulation: RP² x S¹
                * Code automatically generated by dumpConstruction().
                */

                const int adjacencies[3][4] = {
                    { 1, 1, 2, 2},
                    { 0, 0, 2, 2},
                    { 0, 0, 1, 1}
                };

                const int gluings[3][4][4] = {
                    { { 0, 1, 2, 3 }, { 0, 1, 2, 3 }, { 1, 3, 0, 2 }, { 0, 2, 3, 1 } },
                    { { 0, 1, 2, 3 }, { 0, 1, 2, 3 }, { 3, 1, 2, 0 }, { 2, 0, 1, 3 } },
                    { { 2, 0, 3, 1 }, { 0, 3, 1, 2 }, { 3, 1, 2, 0 }, { 1, 2, 0, 3 } }
                };

                Triangulation<3> tri;
                tri.insertConstruction(3, adjacencies, gluings);

                Triangulation<3> after(tri);
                after.zeroTwoMove(after.tetrahedron(0), 0);
                after.intelligentSimplify();
                bool equal = (after.isIsomorphicTo(tri) != nullptr);
                CPPUNIT_ASSERT_MESSAGE("RP² x S¹ : 0-2 move failed to reduce", equal);
            }
            {
                // Test a 0-2 move where neither face adjacent to the edge is part
                // of a gluing
                /**
                * 3-dimensional triangulation: Layered Solid Torus
                * Code automatically generated by dumpConstruction().
                */

                const int adjacencies[1][4] = {
                    { 0, 0, -1, -1}
                };

                const int gluings[1][4][4] = {
                    { { 1, 2, 3, 0 }, { 3, 0, 1, 2 }, { 0, 0, 0, 0 }, { 0, 0, 0, 0 } }
                };

                Triangulation<3> tri;
                tri.insertConstruction(1, adjacencies, gluings);

                Triangulation<3> after(tri);
                after.zeroTwoMove(after.tetrahedron(0), 0);
                after.intelligentSimplify();
                bool equal = (after.isIsomorphicTo(tri) != nullptr);
                CPPUNIT_ASSERT_MESSAGE("LST : 0-2 move on boundary failed to reduce", equal);
            }
            {
                // Test a 0-2 move where only one face adjacent to the edge is part
                // of a gluing
                /**
                * 3-dimensional triangulation: Layered Solid Torus
                * Code automatically generated by dumpConstruction().
                */
                ...
                Triangulation<3> after(tri);
                after.zeroTwoMove(after.tetrahedron(0), 1);
                after.intelligentSimplify();
                bool equal = (after.isIsomorphicTo(tri) != nullptr);
                CPPUNIT_ASSERT_MESSAGE("LST : 0-2 move partially on boundary failed to reduce", equal);
            }
            {
                // Test a 0-2 move where the two adjacent faces are identified
                // together.
                /**
                * 3-dimensional triangulation: Layered Solid Torus
                * Code automatically generated by dumpConstruction().
                */
                ...
                Triangulation<3> after(tri);
                after.zeroTwoMove(after.tetrahedron(0), 5);
                after.intelligentSimplify();
                bool equal = (after.isIsomorphicTo(tri) != nullptr);
                CPPUNIT_ASSERT_MESSAGE("LST : 0-2 move on degree 2 edge failed to reduce", equal);
            }
        }
};
