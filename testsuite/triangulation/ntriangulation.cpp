...
class NTriangulationTest : public TriangulationTest<3> {
    ...
    public:
        ...
        void setUp() {
            ...
            r->join(0, r, NPerm<4>(1, 0, 3, 2));
            r->join(2, r, NPerm<4>(1, 0, 3, 2));
            ...
            r->join(3, s, NPerm<4>(0, 1, 2, 3));
            r->join(2, s, NPerm<4>(0, 3, 1, 2));
            ...
            r->join(2, r, NPerm<4>(0,2));
            r->join(1, s, NPerm<4>(2,0,1,3));
            s->join(2, t, NPerm<4>());
            s->join(1, t, NPerm<4>(2,0,1,3));
            t->join(1, u, NPerm<4>(2,0,1,3));
            u->join(2, u, NPerm<4>(1,2));
            ...
            r->join(2, r, NPerm<4>(2, 3));
            s->join(2, s, NPerm<4>(2, 3));
            t->join(2, t, NPerm<4>(2, 1));
            r->join(1, s, NPerm<4>());
            s->join(0, t, NPerm<4>());
                        ...
                        if (adj) {
                            if (! tet->adjacentTetrahedron(perm[k])) {
                                ...
                                CPPUNIT_FAIL(msg.str());
                                    ...
                                    NPerm<4>::extend(t->adjacentGluing(k)) *
                                    perm.inverse()) {
                                std::ostringstream msg;
                                msg << tri->label()
                                    << ", vertex " << i << ": "
        ...
        void threeSphereRecognition() {
            ...
            tet[0]->join(0, tet[4], NPerm<4>(1,0,2,3));
            tet[0]->join(1, tet[3], NPerm<4>(0,2,3,1));
            tet[0]->join(2, tet[1], NPerm<4>(0,1,3,2));
            tet[0]->join(3, tet[2], NPerm<4>(2,1,3,0));
            tet[1]->join(0, tet[3], NPerm<4>(1,3,2,0));
            tet[1]->join(1, tet[2], NPerm<4>(0,2,3,1));
            tet[1]->join(2, tet[4], NPerm<4>(2,1,0,3));
            tet[2]->join(1, tet[4], NPerm<4>(0,2,3,1));
            tet[2]->join(3, tet[3], NPerm<4>(3,1,2,0));
            tet[3]->join(3, tet[4], NPerm<4>(0,1,2,3));
            delete verifyNotThreeSphere(tri,
        ...
        void solidTorusRecognition() {
            ...
            tet->join(0, tet, NPerm<4>(3, 1, 2, 0));
        ...
        void drillEdge() {
            ...
            // Edge 0 forms a trefoil, and edge 1 is unknotted.
            {
                ...
                tet->join(0, tet, NPerm<4>(1, 2, 3, 0));
                tet->join(2, tet, NPerm<4>(2, 3));
        ...
        void simplification() {
            ...
            tet[2] = tri->newTetrahedron();
            tet[3] = tri->newTetrahedron();
            tet[0]->join(3, tet[2], NPerm<4>());
            tet[0]->join(2, tet[1], NPerm<4>(2, 3));
            tet[3]->join(3, tet[2], NPerm<4>(2, 3));
            tet[3]->join(2, tet[1], NPerm<4>(1, 0));
            if (tri->isValid())
            ...
            tet[1] = tri->newTetrahedron();
            tet[2] = tri->newTetrahedron();
            tet[2]->join(3, tet[2], NPerm<4>(2, 3));
            tet[2]->join(1, tet[1], NPerm<4>(0, 2, 3, 1));
            tet[2]->join(0, tet[0], NPerm<4>(3, 0, 1, 2));
            tet[1]->join(3, tet[0], NPerm<4>(0, 3, 1, 2));
            tet[1]->join(1, tet[0], NPerm<4>());
