...
class SnapPeaTriangulationTest : public CppUnit::TestFixture {
    ...
    public:
        ...
        void setUp() {
            ...
            t->join(0, s, NPerm<4>(0,1,2,3));
            t->join(1, s, NPerm<4>(0,1,2,3));
            t->join(2, s, NPerm<4>(1,3,2,0));
            t->join(3, s, NPerm<4>(1,2,0,3));

            t = flatNor.newTetrahedron();
            s = flatNor.newTetrahedron();
            t->join(0, s, NPerm<4>(0,1,2,3));
            t->join(1, s, NPerm<4>(2,1,0,3));
            t->join(2, s, NPerm<4>(1,3,2,0));
            t->join(3, s, NPerm<4>(2,1,0,3));

            t = degenerateOr.newTetrahedron();
            s = degenerateOr.newTetrahedron();
            t->join(0, t, NPerm<4>(1,0,2,3));
            t->join(2, s, NPerm<4>(1,2,0,3));
            t->join(3, s, NPerm<4>(0,2,3,1));
            s->join(2, s, NPerm<4>(1,2,3,0));

            t = degenerateNor.newTetrahedron();
            s = degenerateNor.newTetrahedron();
            t->join(0, t, NPerm<4>(1,0,2,3));
            t->join(2, s, NPerm<4>(1,2,0,3));
            t->join(3, s, NPerm<4>(0,3,2,1));
            s->join(2, s, NPerm<4>(0,2,3,1));
            ...
            t->join(0, t, NPerm<4>(1,0,3,2));
            t->join(2, t, NPerm<4>(1,0,3,2));
        }
