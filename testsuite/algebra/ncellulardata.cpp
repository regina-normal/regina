...
class NCellularDataTest : public CppUnit::TestFixture {
    ...
    // Comparisons between NTriangulation, Dim4Triangulation and 
    //  NCellularData H1 computations
    ...
    public:
        void setUp() {     
          ...
          copyAndDelete(*knotList[0], NTriangulation::fromIsoSig("cPcbbbiht"));  
            // figure-8 4_1 knot
          knotPolyList[0] = "-1+3t-t^2"; // -t^-1 + 3 - t
          copyAndDelete(*knotList[1], 
            NTriangulation::fromIsoSig("dLQbcccdero")); // 3-twist 5_2 knot
          knotPolyList[1] = "2-3t+2t^2"; // 2t^-1 - 3 + 2t
          copyAndDelete(*knotList[2], 
            NTriangulation::fromIsoSig("eLPkbcddddcwjb")); // Stevedore 6_1
          knotPolyList[2] = "-2+5t-2t^2"; // -2t^-1 + 5 - 2t
          copyAndDelete(*knotList[3], 
            NTriangulation::fromIsoSig("fLLQcbcdeeemgopdp")); // Miller Inst knot 6_2
          knotPolyList[3] = "-1+3t-3t^2+3t^3-t^4"; // -t^-2 + 3t^-1 - 3 + 3t - t^2
          copyAndDelete(*knotList[4], 
            NTriangulation::fromIsoSig("gLLPQccdefffhggaacv")); // 6_3 knot
          ...
          for (unsigned long i=0; i<t4List.size(); i++) 
            t4List[i] = new Dim4Triangulation();
          copyAndDelete(*t4List[0], 
            Dim4Triangulation::fromIsoSig("baa")); // single pentachoron
          copyAndDelete(*t4List[1], 
            Dim4Triangulation::fromIsoSig("cHkbbbRb3asb")); 
            // Poincare dodecahedral space x interval
          copyAndDelete(*t4List[2], 
            Dim4Triangulation::fromIsoSig("cMkabbb+aAa3blb")); 
          ...
          copyAndDelete(*t4List[4], 
            Dim4Triangulation::fromIsoSig("eLAQcbbbdddd0baa0bhahaDaDa"));  
            // another 
          copyAndDelete(*t4List[5], 
            Dim4Triangulation::fromIsoSig("gLLAQQccddeffeffaayaNaNaPbzb0aPbIaxa")); 
            // 6-pentachoron knot
          copyAndDelete(*t4List[6], 
            Dim4Triangulation::fromIsoSig("eAMMcaabccdd+aoa+aAaqbyaca")); } 
            // CP2

          m3List.resize( t3List.size() ); 
          for (unsigned long i=0; i<m3List.size(); i++) 
            m3List[i]=new NCellularData(*t3List[i]);
          m4List.resize( t4List.size() ); 
          for (unsigned long i=0; i<m4List.size(); i++) 
            m4List[i]=new NCellularData(*t4List[i]);          
        ...
        void absolute_h1_comparisons() {
         for (unsigned long i=0; i<m3List.size(); i++) {
          ...
          t3List[i]->getHomologyH1().writeTextShort(str1); 
          std::string S1( str1.str() );  
          NCellularData::GroupLocator h1s( 1, 
            NCellularData::coVariant, NCellularData::STD_coord, 0 );
          std::stringstream str2; 
          m3List[i]->markedGroup( h1s )->writeTextShort(str2); 
          std::string S2( str2.str() );
          NCellularData::GroupLocator h2s( 1, 
            NCellularData::coVariant, NCellularData::DUAL_coord, 0 );
          std::stringstream str3; 
          m3List[i]->markedGroup( h2s )->writeTextShort(str3); 
          std::string S3( str3.str() );
          NCellularData::GroupLocator h3s( 1, 
            NCellularData::coVariant, NCellularData::MIX_coord, 0 );
         ...
         for (unsigned long i=0; i<m4List.size(); i++) {
          std::stringstream str1; 
          t4List[i]->getHomologyH1().writeTextShort(str1); 
          std::string S1( str1.str() );  
          NCellularData::GroupLocator h1s( 1, 
           NCellularData::coVariant, NCellularData::STD_coord, 0 );
          std::stringstream str2; 
    ...
    void noncyclic_TLF_tests() {
      ...
      NTriangulation* tri1(NTriangulation::fromIsoSig(
        "gLLPQaceedffjkxknnn")); 
      NTriangulation* tri2(NTriangulation::fromIsoSig(
        "nLvAAAvPQkcffggghjlkmmlmaaaaaoskkvxisp")); 
      NCellularData cDat1( *tri1 ); 
      NCellularData cDat2( *tri2 ); 
   ...
   void lensspacehomotopyclassification_tests() {
      ...
      for (unsigned long i=0; i<8; i++) // 3 random lens spaces to test.
       {  // p should be at least 2
        NLargeInteger p( maxP.randomBoundedByThis() + 2 ); 
        NLargeInteger q( p.randomBoundedByThis() );
        while ( q.gcd(p) != 1 ) q = p.randomBoundedByThis();
        lens = new NTriangulation(*NLensSpace(p.longValue(),
        ...
        NCellularData::GroupLocator h1L( 1, 
            NCellularData::coVariant, NCellularData::DUAL_coord, 0 );
        NCellularData::FormLocator tifL( 
            NCellularData::torsionlinkingForm, h1L, h1L );
      ...
      void alexpoly_tests() {
       for (unsigned long i=0; i<knotList.size(); i++)
        ...
        if (!((alexPone!=NLargeInteger::one) || 
              (-alexPone!=NLargeInteger::one)))
        ...
        const NMatrixRing< NSVPolynomialRing< NLargeInteger > >
            *cm1(NULL), *cm2(NULL);
        cm1 = knotNCD.alexanderChainComplex( NCellularData::ChainComplexLocator
            (1, NCellularData::DUAL_coord ) );
        cm2 = knotNCD.alexanderChainComplex( NCellularData::ChainComplexLocator
            (2, NCellularData::DUAL_coord ) );
        std::auto_ptr< NMatrixRing< NSVPolynomialRing< NLargeInteger > > > 
          prod((*cm1)*(*cm2));
        if (!prod->isZero()) 
          CPPUNIT_FAIL("NCellularData::alexander module chain complex error "
        ...
        std::auto_ptr< std::list< 
                       regina::NSVPolynomialRing< regina::NLargeInteger > > > 
          ideal( knotNCD.alexanderIdeal() );
        regina::NSVPolynomialRing< regina::NLargeInteger > 
                alexP( ideal->front() );
        // normalize polynomial so that firstAPterm.first = 0
