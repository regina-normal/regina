...
@interface Dim4TriGluingCell : UITableViewCell
...
@implementation Dim4TriGluingCell
@end

#pragma mark - Dim4Triangulation gluings

@interface Dim4TriGluings () <UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, UIActionSheetDelegate> {
...
+ (NSString*)destStringFromFacet:(int)srcFacet dest:(regina::Dim4Pentachoron*)destSimp gluing:(const regina::NPerm<5>&)gluing
{
    if (! destSimp)
...
- (void)editGluingForSimplex:(int)simplex cell:(Dim4TriGluingCell*)cell label:(UILabel*)label
{
    editLabel = label;
    editSimplex = simplex;
    editFacet = label.tag;
...
- (IBAction)touched:(id)sender {
    ...
    Dim4TriGluingCell* cell = static_cast<Dim4TriGluingCell*>([self.pentachora cellForRowAtIndexPath:indexPath]);
...
- (IBAction)elementaryMoves:(id)sender
{
    ...
    UIViewController* sheet = [self.storyboard instantiateViewControllerWithIdentifier:@"Dim4EltMoves"];
    static_cast<Dim4EltMovesController*>(sheet).packet = self.packet;
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    if (editFacet >= 0) {
        ...
        if (dest.length == 0) {
            ...
            regina::NPerm<5> destGluing = regina::NPerm<5>(adj0, adj1, adj2, adj3, (10 - adj0 - adj1 - adj2 - adj3)) *
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.facet2.text = [Dim4TriGluings destStringFromFacet:2 dest:s->adjacentSimplex(2) gluing:s->adjacentGluing(2)];
    cell.facet3.text = [Dim4TriGluings destStringFromFacet:3 dest:s->adjacentSimplex(3) gluing:s->adjacentGluing(3)];
    cell.facet4.text = [Dim4TriGluings destStringFromFacet:4 dest:s->adjacentSimplex(4) gluing:s->adjacentGluing(4)];
    return cell;
}
