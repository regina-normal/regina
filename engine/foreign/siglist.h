...
/*! \file foreign/siglist.h
 *  \brief Allows reading lists of signatures for triangulations or links.
 */

#ifndef __REGINA_FOREIGN_SIGLIST_H
#ifndef __DOXYGEN
#define __REGINA_FOREIGN_SIGLIST_H
#endif
...
/**
 * Reads a list of signatures of triangulations or link diagrams from the given
 * text file, and reconstructs the corresponding mathematical objects in a new
 * packet tree.  The file should contain one signature per line.
 *
 * The signatures may be first-generation or second-generation, and the same
 * file may contain a mix of both.  For example, they could be:
 *
 * - first-generation isomorphism signatures of triangulations, as returned by
 *   `Triangulation<dim>::isoSig()`;
 *
 * - first-generation knot/link signatures, as returned by `Link::knotSig()`;
 *
 ...
 * signatures (if ObjectType is Link), or they could all be signatures for
 * triangulations of some fixed dimension \a dim (if ObjectType is
 * `Triangulation<dim>`).
 *
...
#include "foreign/siglist-impl.h"
