...
const NAbelianGroup* NCellularData::unmarkedGroup( 
...
const NMarkedAbelianGroup* NCellularData::markedGroup( 
                            const GroupLocator &g_desc) const
{
 unsigned long aDim = ( (tri4 != NULL) ? 4 : 3 );
 unsigned long topDim = ( (g_desc.hcs==STD_BDRY_coord) || 
...
const NHomMarkedAbelianGroup* NCellularData::homGroup( 
 ...
 */
const NBilinearForm* NCellularData::bilinearForm( 
                        const FormLocator &f_desc ) const
{
   ...
   if (aDim == 3)
    {
     for (unsigned long i=0; i<ld->getNumberOfInvariantFactors(); i++)
       ...
       for (unsigned long k=0; k<dual_1vec.size(); k++)
        {
         const NTriangle* fac( tri3->getFace( rIx[2][k] ) ); 
           ...
           for (unsigned long k=0; k<dual_1vec.size(); k++)
            {
             ...
             const Dim4Pentachoron* pen( 
                fac->getEmbedding(0).getPentachoron() );
             NPerm5 facinc( fac->getEmbedding(0).getVertices() );
             sum += std_rel_bdry_2vec[k]*dual_1vec[k]*
 ...
 if ( ( f_desc.ft == torsionlinkingForm ) && 
  ...
  { // convienience pairing -- the STD x STD pairing
    ...
    GroupLocator sb( f_desc.rdomain.dim, coVariant, STD_REL_BDRY_coord,
      f_desc.rdomain.cof );
    std::auto_ptr<NHomMarkedAbelianGroup> sc_sb(
        homGroup( HomLocator( sc, sb ) )->torsionSubgroup() ); 
    std::auto_ptr<NHomMarkedAbelianGroup> sc_mc(
        homGroup( HomLocator( sc, mc ) )->torsionSubgroup() );
    std::auto_ptr<NHomMarkedAbelianGroup> dc_mc(
        homGroup( HomLocator( dc, mc ) )->torsionSubgroup() );
    std::auto_ptr<NHomMarkedAbelianGroup> fl( 
...
const NGroupPresentation* NCellularData::groupPresentation( 
...
const NHomGroupPresentation* NCellularData::homGroupPresentation( 
...
unsigned long NCellularData::cellCount( 
...
const NMatrixInt* NCellularData::integerChainComplex( 
...
const NMatrixInt* NCellularData::integerChainMap( 
 ...
 for (std::set<unsigned long>::const_iterator i=maxTreeStd.begin(); 
  ...
  { // this is a local def.
   ...
   if (tri4) if (!tri4->getTetrahedron( nicIx[3][*i] )->isBoundary()) 
                 maxTreedcIx.insert( 
                   dcIxLookup( tri4->getTetrahedron( nicIx[3][*i] ) ) );
  }
  ...
  NCellularData::alexanderPresentationMatrix() const
{
 ...
 const NMatrixRing<NSVPolynomialRing< NLargeInteger > >* 
  N( alexanderChainComplex( ChainComplexLocator(2, NCellularData::DUAL_coord)));
   ...
   const NMarkedAbelianGroup* homol( cdat->markedGroup( 
    ...
    if (tri->isOrientable())
      { // orientable -- we need the torsion linking form
        ...
        { // boundary : orientable
         const NHomMarkedAbelianGroup bhomolMap( *cdat->homGroup( 
          NCellularData::HomLocator( NCellularData::GroupLocator( 1,
