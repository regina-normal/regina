...
/**
 *  Comparison function for polynomials in MVPolynomialRing< Integer >
 */
bool MV_polynomial_comparison( const MVPolynomialRing< Integer > &first, 
                               const MVPolynomialRing< Integer > &second)
{
 ...
 const std::map< PolynomialIndex< signed long >, Integer* > 
    fTerms(first.allTerms()); 
 const std::map< PolynomialIndex< signed long >, Integer* > 
    sTerms(second.allTerms());
 std::map< PolynomialIndex< signed long >, Integer* >::const_iterator 
    fI(fTerms.begin()); 
 std::map< PolynomialIndex< signed long >, Integer* >::const_iterator 
    sI(sTerms.begin()); 
 ...
 * (i1,...,in) --> |i1| + ... + |in|
 */
void recentreNormalize( MVPolynomialRing< Integer > &poly,
     bool plusBias )
{
 // let's get rid of the silly cases
 if (poly.degree() == 0) return; 
 if (poly.degree() == 1) 
  { poly = poly*MVPolynomialRing< Integer >(Integer::one, 
           -poly.allTerms().begin()->first);
    return; 
  }
 std::map< PolynomialIndex< signed long >, Integer* >::const_iterator ci;
 ...
 PolynomialIndex< signed long > Delta(dim); 
 for (unsigned long i=0; i<dim; i++) Delta[i] = -delta[i];
 MVPolynomialRing< Integer > trans( Integer::one, Delta );
 poly = poly * trans;

 // Step 5: ensure the smallest coefficient is positive
 if (*poly.allTerms().begin()->second < 0) poly*=(-Integer::one);
}
...
bool reduceByIdeal( const std::list<MVPolynomialRing< Integer > > &ideal, 
                    MVPolynomialRing< Integer > &elt )
{
...
/*
   ...
   std::vector< MVPolynomialRing< Integer > > indxId;
   std::list< MVPolynomialRing< Integer > >::const_iterator i;
   ...
   if (elt.lastTerm().second % G == Integer::zero) 
    ...
    for (unsigned long i=0; i<gcdV.size(); i++)
      elt -= MVPolynomialRing< Integer >( q*gcdV[i], topD-indxId[i].lastTerm().first )*indxId[i];
    }
   ...
   if ((!didSomething) && laurentPoly) // if no right-reductions worked ,try left-reductions
    {
     ...
     if (elt.lastTerm().second % G == Integer::zero) 
      { 
       ...
       for (unsigned long i=0; i<gcdV.size(); i++)
        elt -= MVPolynomialRing< Integer >( q*gcdV[i], topD-indxId[i].firstTerm().first )*indxId[i];
      }
    }
*/
 }
 ...
 */
/*
void reduceIdealSortStep( std::list< MVPolynomialRing< Integer > > &ideal )
{
 std::list< MVPolynomialRing< Integer > >::iterator i;
 for (i=ideal.begin(); i!=ideal.end(); i++)
  {
   while (i->isZero()) i=ideal.erase(i);  if (i==ideal.end()) break;
   std::pair< signed long, Integer > LT( i->firstTerm() );
   MVPolynomialRing< Integer > opTerm( (LT.second>0) ? Integer::one : -Integer::one, -LT.first );
   (*i) = (*i)*opTerm;
  }
...
/*
void elementaryReductions( std::list< MVPolynomialRing< Integer > > &ideal )
{
 std::list< MVPolynomialRing< Integer > >::iterator i;
 ideal.reverse();
 i = ideal.begin(); 
 while (i!=ideal.end())
  { // check to see if *i can be reduced by testId = ideal \ *i
   std::list< MVPolynomialRing< Integer > > testId; 
   std::list< MVPolynomialRing< Integer > >::iterator j;
   for (j=ideal.begin(); j!=ideal.end(); j++) if (j!=i)
    testId.push_back( *j );
   MVPolynomialRing< Integer > testP( *i );
 ...
 */
/*
void reduceIdeal( std::list< MVPolynomialRing< Integer > > &ideal, bool laurentPoly )
{ 
 ...
 std::vector< MVPolynomialRing< Integer > > vecIdeal; 
 std::list< MVPolynomialRing< Integer > >::const_iterator i; 
 ...
 while (!subSet.atEnd())
  {
   ...
   MVPolynomialRing< Integer > combo;
   for (unsigned long j=0; j<gcdV.size(); j++)
    combo += MVPolynomialRing< Integer >(gcdV[j],maxExp - vecIdeal[subsetV[j]].lastTerm().first)*vecIdeal[subsetV[j]];
 ...
 while (!subSet2.atEnd())
  {
   ...
   MVPolynomialRing< Integer > combo;
   for (unsigned long j=0; j<gcdV.size(); j++)
    combo += MVPolynomialRing< Integer >(gcdV[j],minExp - vecIdeal[subsetV[j]].firstTerm().first)*vecIdeal[subsetV[j]];
   if (!reduceByIdeal( ideal, combo ) ) { ideal.push_back(combo); vecIdeal.push_back(combo); didSomething = true; }
   ++subSet2;
  }
...
/*
bool isSubIdeal( const std::list< MVPolynomialRing< Integer > > &idealA,  
                 const std::list< MVPolynomialRing< Integer > > &idealB )
{
 ...
 std::list< MVPolynomialRing< Integer > >::const_iterator j;
 for (j=idealA.begin(); j != idealA.end(); j++) 
   { MVPolynomialRing< Integer > temp(*j); if (!reduceByIdeal( idealB, temp ) ) return false; }
 return true;
}
*/
