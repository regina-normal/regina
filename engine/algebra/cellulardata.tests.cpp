...
CellularData::homology_coordinate_system & operator++ 
 (CellularData::homology_coordinate_system & cs)
{ return enum_increment(cs, CellularData::first_coord, 
                        CellularData::last_implemented_coord); }

bool CellularData::chainComplexesVerified() const
{
...
bool CellularData::coordinateIsomorphismsVerified(
...
bool CellularData::homologyLESVerified(variance_type var, unsigned long coef) const
{
unsigned long aDim = ( tri3 ? 3 : 4 );
...
bool CellularData::poincareDualityVerified() const
...
for (unsigned long i=0; i <= aDim; i++)
 {
  ...
  const HomMarkedAbelianGroup duality( 
...
bool CellularData::intersectionFormsVerified() const
{ 
bool retval=true;
unsigned long aDim = 3;
unsigned long coeff = 0;
