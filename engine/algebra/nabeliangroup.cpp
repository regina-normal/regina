...
NAbelianGroup::NAbelianGroup(const NMatrixInt& M, const NMatrixInt& N)
{
	...
	unsigned long lim = (tempN.rows() < tempN.columns() ? tempN.rows() : tempN.columns() );
	...
	for (unsigned long i=0; i<lim; i++)
	 {
	  if (tempN.entry(i,i) != 0)
           {
	    rank--;
	    if (tempN.entry(i,i) > 1) torsion.insert(tempN.entry(i,i));
           }
	 }
	...
	metricalSmithNormalForm(tempM);
        ...
        for (unsigned long i=0; i<lim; ++i) 
	 {
	  if (tempM.entry(i,i) != 0) rank --;
	 }
}

NAbelianGroup::NAbelianGroup(const NMatrixInt& M, const NMatrixInt& N, const NLargeInteger &p)
{
	...
	if (cof == 0)
	 {
          for (unsigned long i=0; i<lim; i++) if (tempN.entry(i,i) != 0)
	     {
	      rank--;
	      if (tempN.entry(i,i) > 1) torsion.insert(tempN.entry(i,i));
             }
	...
	if (cof != 0)
	 {
	  for (unsigned long i=0; i<rank; i++) torsion.insert(cof);
	  rank = 0;
	 }	
	addTorsionElements(torsion);
}
