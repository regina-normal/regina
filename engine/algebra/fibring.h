...
class REGINA_API NMapToS1 {
 ...
 //  j of this edge of vertex vert. 
   std::map< Face<2,0>*, std::list< std::pair< Face<2,1>*, unsigned long > > > 
    dim2inc;
   std::map< Face<3,0>*,    std::list< std::pair< Face<3,1>*,    unsigned long > > > 
    dim3inc;
   std::map< Face<4,0>*, std::list< std::pair< Face<4,1>*, unsigned long > > > 
    dim4inc;
  ...
  */ 
 NMapToS1( const Triangulation<2>* tri );

 /**
  * 3-manifold constructor for the class. 
  */ 
 NMapToS1( const Triangulation<3>* tri ); 

 /**
  * 4-manifold constructor for the class. 
  */ 
 NMapToS1( const Triangulation<4>* tri );
  ...
  * fibre bundle.
  */
 bool verifyPrimitiveH1( const std::vector<Rational> &cocy ) const;
  ...
  */
void triangulateFibre( const std::vector<Rational> &cocy, 
 Dim1Triangulation** TRI1=NULL, 
 // TODO: replace with Triangulation<1> ?? look into this.
 Triangulation<2>** TRI2=NULL,  
 Triangulation<3>**    TRI3=NULL );
