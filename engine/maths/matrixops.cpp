...
unsigned rowBasis(NMatrixInt& matrix) {
    ...
    while (doneRows < rank) {
        ...
        if (c == n) {
            ...
            // some very large matrix entries, though we're using LargeInteger
...
void columnEchelonForm(NMatrixInt &M, NMatrixInt &R, NMatrixInt &Ri,
    ...
    LargeInteger d,r; // given two LargeIntegers a and b, we will represent
                       // a/b by d and a % b by r in the algorithm.
    LargeInteger u,v,gcd, a,b; // for column operations u,v,a,b represent
                                // a 2x2 matrix.
    LargeInteger tmp;
...
std::unique_ptr<NMatrixInt> preImageOfLattice(const NMatrixInt& hom,
        const std::vector<LargeInteger>& L) {
...
std::unique_ptr<NMatrixInt> torsionAutInverse(const NMatrixInt& input,
        const std::vector<LargeInteger> &invF) {
    ...
    while (wRow > 0)
    {
        ...
        LargeInteger R; // divisionAlg needs a remainder so we give it one, 
        ...
        while (wCol > 0)
        {
            ...
            for (unsigned long i=0; i<workMat.rows(); i++)
            {
             ...
             LargeInteger W(workMat.entry(i, wCol)), 
    ...
    {
        LargeInteger X(workMat.entry(j, i)); 
...
bool metricFindPivot(const unsigned long &currStage, const NMatrixInt &matrix, 
        ...
        const std::vector<LargeInteger> &rowNorm,
        const std::vector<LargeInteger> &colNorm, 
        const std::vector<LargeInteger> &rowGCD) {
    bool pivotFound = false;
    // find the smallest positive rowGCD
    LargeInteger SProwGCD(LargeInteger::zero);
...
void metricSwitchRows(const unsigned long &currStage, const unsigned long &i, 
        ...
        std::vector<LargeInteger> &rowNorm, std::vector<LargeInteger> &rowGCD)
{
...
void metricSwitchCols(const unsigned long &currStage, const unsigned long &i, 
        ...
        std::vector<LargeInteger> &colNorm)
{
...
void metricColOp(const unsigned long &currStage, const unsigned long &i,
        ...
        const LargeInteger a, const LargeInteger b, 
        const LargeInteger c, const LargeInteger d, 
        NMatrixInt *rowBasis, NMatrixInt *rowBasisInv, 
        std::vector<LargeInteger> &rowNorm, 
        std::vector<LargeInteger> &colNorm)
{
    LargeInteger t1, t2;
    // smart rowMetric recomputation and transformation
    colNorm[i] = LargeInteger::zero; colNorm[j] = LargeInteger::zero;
    for (unsigned long k=currStage; k<matrix.rows(); k++)
    {
...
void metricRowOp(const unsigned long &currStage, const unsigned long &i, 
    ...
    std::vector<LargeInteger> &rowNorm, std::vector<LargeInteger> &colNorm,
    std::vector<LargeInteger> &rowGCD)
{
    LargeInteger t1, t2;
    // smart norm recomputation and transformation
    rowNorm[i] = LargeInteger::zero; rowNorm[j] = LargeInteger::zero;
    rowGCD[i] = LargeInteger::zero;  rowGCD[j] = LargeInteger::zero;
    for (unsigned long k=currStage; k<matrix.columns(); k++)
    {
...
void metricalSmithNormalForm(NMatrixInt& matrix,
    ...
    // set up metrics. 
    std::vector<LargeInteger> rowNorm(matrix.rows(), LargeInteger::zero);
    std::vector<LargeInteger> colNorm(matrix.columns(), LargeInteger::zero);
    std::vector<LargeInteger> rowGCD(matrix.rows(), LargeInteger::zero);
...
rowMuckerLoop: 
     ...
     //  rowGCD[this row],  pick the row with the lowest of these gcds...
            ...
            LargeInteger bestGCD(matrix.entry(currStage, currStage).abs());       
            for (i=currStage+1; i<matrix.rows(); i++)       
            {
            ...
            if ( rowT > currStage )
            {
                metricRowOp(currStage, currStage, rowT, matrix, 
                        ...
                        LargeInteger::zero, LargeInteger::one, colSpaceBasis, 
