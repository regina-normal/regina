...
NTriangulation* NSnapPeaCensusManifold::construct() const {
    ...
    if (section_ == SEC_5) {
        if (index_ == 0) {
            ...
            r->join(0, s, NPerm<4>(0, 1, 3, 2));
            r->join(1, s, NPerm<4>(2, 3, 1, 0));
            r->join(2, s, NPerm<4>(3, 2, 1, 0));
            r->join(3, s, NPerm<4>(1, 0, 3, 2));
        } else if (index_ == 2) {
            ans = new NTriangulation();
            NTetrahedron* r = ans->newTetrahedron();
            NTetrahedron* s = ans->newTetrahedron();
            r->join(0, s, NPerm<4>(0, 1, 3, 2));
            r->join(1, s, NPerm<4>(3, 1, 2, 0));
            r->join(2, s, NPerm<4>(2, 1, 3, 0));
            r->join(3, s, NPerm<4>(3, 1, 0, 2));
        } else if (index_ == 3) {
            ans = new NTriangulation();
            NTetrahedron* r = ans->newTetrahedron();
            NTetrahedron* s = ans->newTetrahedron();
            r->join(0, s, NPerm<4>(0, 1, 3, 2));
            r->join(1, s, NPerm<4>(2, 1, 0, 3));
            r->join(2, s, NPerm<4>(0, 3, 2, 1));
            r->join(3, s, NPerm<4>(1, 0, 2, 3));
        } else if (index_ == 4) {
