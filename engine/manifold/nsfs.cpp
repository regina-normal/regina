...
NTriangulation* NSFSpace::construct() const {
    ...
    a->join(1, b, NPerm<4>());
    b->join(2, c, NPerm<4>());
    c->join(3, a, NPerm<4>(1, 2, 3, 0));

    std::list<NSFSFibre>::const_iterator fit = fibres_.begin();
    NSatAnnulus(a, NPerm<4>(1, 0, 2, 3), b, NPerm<4>(1, 2, 0, 3)).
        attachLST(ans, fit->alpha, fit->beta);
    fit++;
    NSatAnnulus(b, NPerm<4>(2, 1, 3, 0), c, NPerm<4>(2, 3, 1, 0)).
        attachLST(ans, fit->alpha, fit->beta);
    fit++;
    ...
    while (fit != fibres_.end()) {
        ...
        b = ans->newTetrahedron();
        c = ans->newTetrahedron();
        a->join(3, prevA, NPerm<4>(2, 3));
        b->join(3, prevC, NPerm<4>(0, 2, 3, 1));
        a->join(1, b, NPerm<4>());
        b->join(2, c, NPerm<4>());
        c->join(3, a, NPerm<4>(1, 2, 3, 0));
    ...
    NSatAnnulus(a, NPerm<4>(1, 0, 3, 2), c, NPerm<4>(2, 3, 0, 1)).attachLST(ans,
