...
bool FacetPairingBase<dim>::isCanonicalInternal(
    ...
    if (isUnmatched(0, 0)) {
        ...
        for (int i = 0; i < NPerm<dim+1>::nPerms; ++i) {
            ...
            ans->simpImage(0) = 0;
            ans->facetPerm(0) = NPerm<dim+1>::atIndex(i);
            list.push_back(ans);
        }
        return true;
    }
...
template <int dim>
Isomorphism<dim>* FacetPairingBase<dim>::makeCanonical() {
    // Lazy helper define.
#define toInt(fs) ((fs.simp) * (dim + 1) + (fs.facet))
    ...
    if ( startSimplex == size_ ) {
        ...
        ans = new Isomorphism<dim>(size_);
        for ( int i = 0; i < size_ ; i++ ) {
            ans->simpImage(i) = i;
            ans->facetPerm(i) = NPerm<dim+1>::Sn[0];
        }
        return ans;
    }
