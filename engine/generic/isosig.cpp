...
template <int dim>
std::string isoSig(const typename DimTraits<dim>::Triangulation& tri,
        unsigned simp, const typename DimTraits<dim>::Perm& vertices) {
    ...
    for (it = tri.getComponents().begin(), i = 0;
        ...
        for (simp = 0; simp < (*it)->getNumberOfSimplices(); ++simp)
            for (perm = 0; perm < Perm::nPerms; ++perm) {
                curr = isoSig<dim>(tri, (*it)->getSimplex(simp)->markedIndex(),
                    Perm::orderedSn[perm]);
...
template <int dim>
typename DimTraits<dim>::Triangulation* fromIsoSig(const std::string& sig) {
    typedef typename DimTraits<dim>::Perm Perm;
    typedef typename DimTraits<dim>::Simplex Simplex;
    typedef typename DimTraits<dim>::Triangulation Triangulation;
...
template std::string isoSig<2>(const Dim2Triangulation&);
template std::string isoSig<3>(const NTriangulation&);

template Dim2Triangulation* fromIsoSig<2>(const std::string&);
template NTriangulation* fromIsoSig<3>(const std::string&);
