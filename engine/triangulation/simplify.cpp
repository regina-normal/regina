...
namespace {
    ...
    //     each i.
    const NPerm<4> threeTwoVertices[3] = {
        NPerm<4>(3,1,2,0), NPerm<4>(3,2,0,1), NPerm<4>(3,0,1,2)
    };
    ...
    //     each i.
    const NPerm<4> twoThreeVertices[2] = {
        NPerm<4>(1,2,3,0), NPerm<4>(0,2,3,1)
    };
...
bool NTriangulation::threeTwoMove(NEdge* e, bool check, bool perform) {
    ...
    newTet[0]->join(3, newTet[1], NPerm<4>());
...
bool NTriangulation::twoThreeMove(NTriangle* f, bool check, bool perform) {
    ...
    NPerm<4> internalPerm = NPerm<4>(0,1,3,2);
...
bool NTriangulation::oneFourMove(NTetrahedron* tet, bool /* check */,
    ...
    for (i = 0; i < 4; ++i)
        for (j = i + 1; j < 4; ++j)
            newTet[i]->join(j, newTet[j], NPerm<4>(i, j));
...
bool NTriangulation::twoZeroMove(NEdge* e, bool check, bool perform) {
    ...
    // other.
    NPerm<4> crossover = tet[0]->adjacentGluing(perm[0][2]);
    NPerm<4> gluing;
    NTetrahedron* top;
    NTetrahedron* bottom;
    int topFace;
...
bool NTriangulation::twoZeroMove(NVertex* v, bool check, bool perform) {
    ...
    if (! top) {
        ...
        NPerm<4> gluing = tet[1]->adjacentGluing(vertex[1]) *
            crossover * top->adjacentGluing(topFace);
        tet[0]->unjoin(vertex[0]);
        tet[1]->unjoin(vertex[1]);
        top->join(topFace, bottom, gluing);
...
bool NTriangulation::twoOneMove(NEdge* e, int edgeEnd,
    ...
    NPerm<4> oldVertices = emb.vertices();
    ...
    if (! adjTet[0])
        ...
        NPerm<4> gluing = top->adjacentGluing(topGlued[1])
            * NPerm<4>(topGlued[0], topGlued[1])
    ...
    newTet->join(2, newTet, NPerm<4>(2,3));
    ...
    if (oldTet->adjacentTetrahedron(oldVertices[otherEdgeEnd]) == top) {
        ...
        NPerm<4> bottomFacePerm = NPerm<4>(oldVertices[edgeEnd],
            oldVertices[otherEdgeEnd], oldVertices[2], oldVertices[3]);
        NPerm<4> gluing = bottomFacePerm.inverse() *
            top->adjacentGluing(topFace) * bottomToTop *
            bottomFacePerm * NPerm<4>(0,1);
        top->unjoin(topFace);
        ...
        if (adjTop) {
            NPerm<4> topGluing = top->adjacentGluing(topFace) *
                bottomToTop * bottomFacePerm * NPerm<4>(0,1);
            top->unjoin(topFace);
            newTet->join(0, adjTop, topGluing);
        }
        if (adjBottom) {
            NPerm<4> bottomGluing = oldTet->adjacentGluing(bottomFace) *
                bottomFacePerm;
...
bool NTriangulation::closeBook(NEdge* e, bool check, bool perform) {
    ...
    NPerm<4> p0 = front.vertices();
    NPerm<4> p1 = back.vertices();
    ...
    t0->join(p0[3], t1, p1 * NPerm<4>(2, 3) * p0.inverse());
    return true;
}
...
bool NTriangulation::collapseEdge(NEdge* e, bool check, bool perform) {
    ...
    NPerm<4>* embVertices = new NPerm<4>[degree];
    ...
    for (i = 0; i < degree; ++i) {
        ...
        if (top && bot)
            top->join(topPerm[embVertices[i][0]], bot,
                botPerm * NPerm<4>(embVertices[i][0], embVertices[i][1]) *
                topPerm.inverse());
