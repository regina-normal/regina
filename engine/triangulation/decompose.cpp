...
long NTriangulation::connectedSumDecomposition(Packet* primeParent,
    ...
    while (finalZ++ < initZ) {
        ...
        if (initOrientable) {
            ...
            t0->join(0, t1, NPerm<4>(0, 1, 3, 2));
            t0->join(1, t1, NPerm<4>(0, 1, 3, 2));
            t0->join(2, t1, NPerm<4>(1, 3, 2, 0));
            t0->join(3, t1, NPerm<4>(2, 0, 1, 3));
        }
...
bool NTriangulation::hasSimpleCompressingDisc() const {
    ...
    for (tit = use.simplices_.begin(); tit != use.simplices_.end(); ++tit) {
        ...
        tet->join(NEdge::edgeVertex[equator][0], tet, NPerm<4>(
