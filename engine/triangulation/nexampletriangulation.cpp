...
NTriangulation* NExampleTriangulation::bingsHouse() {
    ...
    r->join(0, r, NPerm<4>(0, 1));
    s->join(0, s, NPerm<4>(0, 1));
    r->join(2, s, NPerm<4>(3, 1, 2, 0));
    s->join(3, r, NPerm<4>(2, 1, 0, 3));
...
NTriangulation* NExampleTriangulation::rp2xs1() {
    ...
    r->join(1, t, NPerm<4>(2, 3, 0, 1));
    r->join(3, t, NPerm<4>(2, 3, 0, 1));
...
NTriangulation* NExampleTriangulation::solidKleinBottle() {
    ...
    s->join(0, r, NPerm<4>(0, 1, 2, 3));
    s->join(3, r, NPerm<4>(3, 0, 1, 2));
    s->join(1, t, NPerm<4>(3, 0, 1, 2));
    s->join(2, t, NPerm<4>(0, 1, 2, 3));
...
NTriangulation* NExampleTriangulation::figureEight() {
    ...
    NTetrahedron* r = ans->newTetrahedron();
    NTetrahedron* s = ans->newTetrahedron();
    r->join(0, s, NPerm<4>(1, 3, 0, 2));
    r->join(1, s, NPerm<4>(2, 0, 3, 1));
    r->join(2, s, NPerm<4>(0, 3, 2, 1));
    r->join(3, s, NPerm<4>(2, 1, 0, 3));
...
NTriangulation* NExampleTriangulation::trefoil() {
    ...
    NTetrahedron* r = ans->newTetrahedron();
    NTetrahedron* s = ans->newTetrahedron();
    r->join(0, s, NPerm<4>(2, 3));
    r->join(1, s, NPerm<4>(2, 3));
    r->join(2, s, NPerm<4>(1, 3));
    r->join(3, s, NPerm<4>(0, 2));
...
NTriangulation* NExampleTriangulation::gieseking() {
    ...
    NTetrahedron* r = ans->newTetrahedron();
    r->join(0, r, NPerm<4>(1, 2, 0, 3));
    r->join(2, r, NPerm<4>(0, 2, 3, 1));
...
NTriangulation* NExampleTriangulation::cuspedGenusTwoTorus() {
    ...
    r->join(0, s, NPerm<4>());
    r->join(1, t, NPerm<4>(1, 2, 3, 0));
    r->join(2, u, NPerm<4>(1, 0, 3, 2));
    s->join(3, t, NPerm<4>());
    t->join(1, u, NPerm<4>());
    ans->finiteToIdeal();
