...
void NTriangulation::barycentricSubdivision() {
    ...
    for (tet=0; tet < nOldTet; ++tet)
        for (permIdx = 0; permIdx < 24; ++permIdx) {
            perm = NPerm<4>::S4[permIdx];
            // (0, 1, 2, 3) -> (face, edge, vtx, corner)

            // Internal gluings within the old tetrahedron:
            newTet[24 * tet + permIdx]->join(perm[3],
                newTet[24 * tet + (perm * NPerm<4>(3, 2)).S4Index()],
                NPerm<4>(perm[3], perm[2]));

            newTet[24 * tet + permIdx]->join(perm[2],
                newTet[24 * tet + (perm * NPerm<4>(2, 1)).S4Index()],
                NPerm<4>(perm[2], perm[1]));

            newTet[24 * tet + permIdx]->join(perm[1],
                newTet[24 * tet + (perm * NPerm<4>(1, 0)).S4Index()],
                NPerm<4>(perm[1], perm[0]));
    ...
    for (i=0; i<numOldTet; i++) {
        ...
        for (j=0; j<4; j++)
            newTet[tip[j] + i * nDiv]->join(j,
                newTet[interior[j] + i * nDiv], NPerm<4>());
            ...
            for (k=0; k<4; k++)
                if (j != k) {
                    newTet[interior[j] + i * nDiv]->join(k,
                        newTet[vertex[k][j] + i * nDiv], NPerm<4>());
                }
        }
            ...
            for (k=0; k<4; k++)
                if (j != k) {
                    newTet[edge[j][k] + i * nDiv]->join(j,
                        newTet[edge[k][j] + i * nDiv], NPerm<4>(j,k));

                    for (l=0; l<4; l++)
                        if ( (l != j) && (l != k) )
                            newTet[edge[j][k] + i * nDiv]->join(l,
                                newTet[vertex[j][l] + i * nDiv], NPerm<4>(k,l));
                }
    }
...
void NTriangulation::puncture(NTetrahedron* tet) {
    ...
    prism[0][0]->join(0, prism[0][1], NPerm<4>(3,0,1,2));
    prism[0][1]->join(0, prism[0][2], NPerm<4>(3,0,1,2));

    prism[1][0]->join(1, prism[1][1], NPerm<4>(3,0,1,2));
    prism[1][1]->join(1, prism[1][2], NPerm<4>(3,2,0,1));

    prism[0][0]->join(1, prism[1][0], NPerm<4>(1,2,3,0));
    prism[0][0]->join(2, prism[1][0], NPerm<4>(1,2,3,0));
    prism[0][1]->join(1, prism[1][1], NPerm<4>(1,2,3,0));
    prism[0][1]->join(2, prism[1][1], NPerm<4>(1,2,3,0));
    prism[0][2]->join(1, prism[1][2], NPerm<4>(0,1,3,2));
    prism[0][2]->join(2, prism[1][2], NPerm<4>(0,1,3,2));

    NTetrahedron* adj = tet->adjacentTetrahedron(0);
    if (adj) {
        NPerm<4> gluing = tet->adjacentGluing(0);
        tet->unjoin(0);
        prism[1][0]->join(0, adj, gluing);
    }

    tet->join(0, prism[0][0], NPerm<4>(3,0,1,2));
}
...
void NTriangulation::connectedSumWith(const NTriangulation& other) {
    ...
    bdry[0][0]->join(0, bdry[1][0], NPerm<4>(2, 3));
    bdry[0][1]->join(0, bdry[1][1], NPerm<4>(2, 3));
}
