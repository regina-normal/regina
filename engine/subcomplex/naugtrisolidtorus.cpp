...
NAugTriSolidTorus* NAugTriSolidTorus::isAugTriSolidTorus(
    ...
    if (nTet == 3) {
        ...
        for (i = 0; i < 24; i++) {
            ...
            if (NPerm4::S4[i][0] > NPerm4::S4[i][3])
                continue;

            core = NTriSolidTorus::formsTriSolidTorus(base, NPerm4::S4[i]);
            if (core) {
    ...
    NPerm4 swap3Top(3, topCoreFace);
    NPerm4 swap23(2, 3);
    NTriSolidTorus* core;
    NTetrahedron* coreTets[3];
    NPerm4 coreVertexRoles[3];
    int whichLayered[3];
    int usedLayered;
        ...
        if (core) {
            ...
            for (j = 0; j < 3; j++) {
                ...
                if (core->isAnnulusSelfIdentified(j, &q)) {
                    ...
                    switch (q[0]) {
                        case 0:
                            edgeGroupRoles[j] = NPerm4(2, 0, 1, 3); break;
                        case 2:
                            edgeGroupRoles[j] = NPerm4(1, 2, 0, 3); break;
                        case 3:
                            edgeGroupRoles[j] = NPerm4(0, 1, 2, 3); break;
                    }
                } else {
