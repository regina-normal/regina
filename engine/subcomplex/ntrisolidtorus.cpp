...
bool NTriSolidTorus::isAnnulusSelfIdentified(int index, NPerm<4>* roleMap) const {
    int lower = (index + 1) % 3;
    int upper = (index + 2) % 3;
...
unsigned long NTriSolidTorus::areAnnuliLinkedMajor(int otherAnnulus) const {
    ...
    if (adj == tet[0] || adj == tet[1] || adj == tet[2] || adj == 0)
        return 0;
    NPerm<4> roles = tet[right]->adjacentGluing(
        vertexRoles_[right][1]) * vertexRoles_[right] * NPerm<4>(2, 3, 1, 0);
    if (roles != tet[left]->adjacentGluing(
            vertexRoles_[left][2]) * vertexRoles_[left] * NPerm<4>(3, 2, 0, 1))
        return 0;
...
unsigned long NTriSolidTorus::areAnnuliLinkedAxis(int otherAnnulus) const {
    ...
    if (adj == tet[0] || adj == tet[1] || adj == tet[2] || adj == 0)
        return 0;
    NPerm<4> roles = tet[right]->adjacentGluing(
        vertexRoles_[right][1]) * vertexRoles_[right] * NPerm<4>(2, 1, 0, 3);
    ...
    NPerm<4> topRoles(chain.topVertexRoles());
    ...
    if (topRoles != tet[left]->adjacentGluing(
            vertexRoles_[left][2]) * vertexRoles_[left] * NPerm<4>(3, 0, 1, 2))
        return 0;
...
NTriSolidTorus* NTriSolidTorus::formsTriSolidTorus(NTetrahedron* tet,
    ...
    if (ans->tet[1]->adjacentGluing(roles1[0]) * roles1 *
            NPerm<4>(1, 2, 3, 0) != ans->vertexRoles_[2]) {
        delete ans;
        return 0;
    }
