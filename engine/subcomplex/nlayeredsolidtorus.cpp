...
NLayeredSolidTorus* NLayeredSolidTorus::formsLayeredSolidTorusTop(
    ...
    NTetrahedron* next;
    NPerm<4> cross1, cross2;
    NPerm<4> canon1, canon2;
    NFacePair pair = NFacePair(topFace1, topFace2).complement();
    NPerm<4> vRoles(pair.upper(), topFace1, topFace2, pair.lower());
    NPerm<4> topRoles(vRoles);
    NPerm<4> nextRoles;
    ...
    while (true) {
        ...
        nextRoles = NPerm<4>(pair.upper(), cross1[vRoles[0]], cross2[vRoles[3]],
            pair.lower());
        ...
        if (canon2 == NPerm<4>(0, 1, 3, 2)) {
            ...
            y_ = y + z;
            z_ = z;
        } else if (canon2 == NPerm<4>(0, 3, 1, 2)) {
            // 012 -> 031.
            // old a = a+b
            // old b = a
            ...
            y_ = y + z;
            z_ = y;
        } else if (canon2 == NPerm<4>(1, 0, 3, 2)) {
            // 012 -> 103.
            // old a = -a
            // old b = b
            ...
            y_ = -y;
            z_ = z;
        } else if (canon2 == NPerm<4>(1, 3, 0, 2)) {
            // 012 -> 130.
            // old a = b
            // old b = -a
            ...
            y_ = -z;
            z_ = y;
        } else if (canon2 == NPerm<4>(3, 0, 1, 2)) {
            // 012 -> 301.
            // old a = -(a+b)
            // old b = -b
            ...
            y_ = -y;
            z_ = -y - z;
        } else if (canon2 == NPerm<4>(3, 1, 0, 2)) {
            // 012 -> 310.
            // old a = -b
            // old b = -(a+b)
...
NTriangulation* NLayeredSolidTorus::flatten(const NTriangulation* original,
    ...
    if (adj0 && adj1 && (adj0 != newTop)) {
        ...
        NPerm<4> groups1 = NPerm<4>(topFace_[0], topFace_[1]) *
            NPerm<4>(underFaces.lower(), underFaces.upper()) * groups0;
        ...
        adj0->join(groups0[3], adj1, groups1 *
            NPerm<4>((mobiusBandBdry + 1) % 3, (mobiusBandBdry + 2) % 3) *
            groups0.inverse());
    }
