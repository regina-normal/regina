...
Dim4Isomorphism* ordering_iso(const Dim4Triangulation
                              &trig) // , bool force_oriented)
{
// false for remain fixed, true for reverse
    NPartition edgeOr( trig.getNumberOfEdges(), 0, false );
    for ( ; !edgeOr.atEnd();
            ++edgeOr ) { // iterate through all possible edge orientations.
        ...
        for (unsigned long i=0; i<trig.getNumberOfPentachora(); i++) {
            ...
            if (inCount.size()
                    <5) { // invalid! must be exactly 5 distinct inward-pointing edge counts
                validLabel = false;
                break;
            }
        }
        if (validLabel) {
            ...
            for (unsigned long i=0; i<trig.getNumberOfPentachora(); i++) {
                ...
                retval->facetPerm(i)=NPerm5(relabel[0], relabel[1], relabel[2], relabel[3],
                                            relabel[4]);
            }
            return retval;
        }
...
bool Dim4Triangulation::order()
{
    ...
    if(! isOrdered()) {
        std::cout<<"Dim4Triangulation::order() error 1."<<std::endl;
        exit(1);
    }
    //if(forceOriented && ! isOrientable())
