        ...
        if (!getTetrahedron(i)->isBoundary()) {
            ...
            for (unsigned long j=0; j<4; j++) {
                ...
                for (unsigned long k=1; k<4; k++)
                    if (aTet->getVertex( (j+k) % 4 )->isIdeal()) {
                        tIV=true;
                        break;
                    }
                {
                    subDivNot p0( _CCt, pentachoronIndex(
                                      tEmb0.getPentachoron() ) ); // might have to
                    subDivNot p1( _CCt, pentachoronIndex(
                                      tEmb1.getPentachoron() ) ); // modify later
    ...
    for (unsigned long i=0; i<getNumberOfPentachora(); i++) {
        ...
        for (unsigned long j=0; j<5; j++) {
            ...
            for (unsigned long k=0; k<4; k++) {
                ...
                if (aTri->getVertex(0)->isIdeal()
                        && aTri->getVertex(2)->isIdeal()) { // glue 0 to 2
                    p0.penType = _CCdt;
                    p1.penType = _CCdt;
                    p0.vtxIdx = 2;
                    p1.vtxIdx = 0;
                ...
                if (aTri->getVertex(0)->isIdeal()
                        && !aTri->getVertex(2)->isIdeal()) { // glue 0 CCdt to CCt
                    p0.penType = _CCdt;
                    p1.penType = _CCt;
                    p0.vtxIdx = 0;
                    p1.vtxIdx = 0;
        ...
        for (unsigned long j=0; j<5; j++) {
            ...
            for (unsigned long k=0; k<6; k++) {
                ...
                if (eMap[3]==triInc2[0] ) { // id vtx 0 in tri2 adj
                    glueT=triInc2[0];
                    p0.penType = _CCt;
                } else if (!aTet->getVertex( triInc2[1] )->isIdeal() &&
                    ...
                    if (!aTet->getVertex( triInc2[0] )->isIdeal() &&
                        ...
                        if (aTet->getVertex( triInc2[1] )->isIdeal() &&
                            ...
                            incPerm0 = triInc2;
                            p0.vtxIdx = 1;
                        } else if ( aTet->getVertex( triInc2[2] )->isIdeal() &&
                            ...
                            incPerm0 = triInc2;
                            p0.vtxIdx = 2;
                        } else if ( aTet->getVertex( triInc2[0] )->isIdeal() &&
                                    eMap[3]==triInc2[1] ) {
                            glueT=0;
                            p0.penType = _CCdt;
                            incPerm0 = NPerm5(triInc2[1],triInc2[2],triInc2[0], triInc2[3], triInc2[4] );
                        } else {
                            std::cout<<"Dim4Triangulation::idealToFinite() Error 1."<<std::endl;
                            exit(1);
                        }
                if (eMap[2]==triInc3[0]) { // repeating the above for the other side.
                    p1.penType = _CCt;
                } else if (!aTet->getVertex( triInc3[1] )->isIdeal() &&
                    ...
                    if (!aTet->getVertex( triInc3[0] )->isIdeal() &&
                        ...
                        if (aTet->getVertex( triInc3[1] )->isIdeal() &&
                            ...
                            incPerm1 = triInc3;
                            p1.vtxIdx = 1;
                        } else if ( aTet->getVertex( triInc3[2] )->isIdeal() &&
                                    !aTet->getVertex( triInc3[0] )->isIdeal() &&
                                    eMap[2]==triInc3[1] ) {
                            p1.penType = _CCdt;
                            incPerm1 = triInc3;
                            p1.vtxIdx = 2;
                        } else if ( aTet->getVertex( triInc3[0] )->isIdeal() &&
                                    eMap[2]==triInc3[1] ) {
                            p1.penType = _CCdt;
                            incPerm1 = NPerm5(triInc3[1],triInc3[2],triInc3[0], triInc3[3], triInc3[4] );
                        } else {
                            std::cout<<"Dim4Triangulation::idealToFinite() Error 2."<<std::endl;
                            exit(1);
                        }
#ifdef DEBUG // test to check if p0 and p1 exist 
                ...
                if (newPens[p1]->adjacentPentachoron( incPerm1.inverse()[NPerm5(eMap[2],
                                                      eMap[3])[incPerm0[glueT]]] )!=NULL)
...
#endif // and if gluings previously set. 
                    newPens[ p0 ]->joinTo( (j+k)%5, newPens[ p1 ], tetInc.inverse()*NPerm5(j,
                                           (j+k)%5) );
                }
            }
