...
namespace {
    ...
    class Bdry {
        ...
        protected:
             ...
             */
            Bdry(Block* block, NPerm<4> outerVertices);
    };
    ...
    class BdryQuad : public Bdry {
        ...
        private:
             ...
             * the outer tetrahedron.
             */
            BdryQuad(Block* block, NPerm<4> outerVertices);
    ...
    class BdryHex : public Bdry {
        ...
        private:
             ...
             * the outer tetrahedron.
             */
            BdryHex(Block* block, NPerm<4> outerVertices);
    ...
    TriPrism::TriPrism(const NTetrahedron *outerTet, int type,
        ...
        innerTet_[1]->join(1, innerTet_[0], NPerm<4>());
        innerTet_[1]->join(3, innerTet_[2], NPerm<4>());

        NPerm<4> vertices = NPerm<4>(0, type);

        BdryQuad* q;

        bdry_[vertices[0]] = 0;

        q = new BdryQuad(this, vertices * NPerm<4>(0, 2, 3, 1));
        q->innerTet_[0] = innerTet_[1];
        q->innerTet_[1] = innerTet_[2];
        q->innerVertices_[0] = NPerm<4>(2, 3, 1, 0);
        q->innerVertices_[1] = NPerm<4>(1, 3, 2, 0);
        bdry_[vertices[1]] = q;

        q = new BdryQuad(this, vertices * NPerm<4>(2, 3));
        q->innerTet_[0] = innerTet_[0];
        q->innerTet_[1] = innerTet_[2];
        q->innerVertices_[0] = NPerm<4>(2, 1, 0, 3);
        q->innerVertices_[1] = NPerm<4>(0, 3, 2, 1);
        bdry_[vertices[2]] = q;

        q = new BdryQuad(this, vertices);
        q->innerTet_[0] = innerTet_[0];
        q->innerTet_[1] = innerTet_[1];
        q->innerVertices_[0] = NPerm<4>(3, 1, 0, 2);
        q->innerVertices_[1] = NPerm<4>(0, 1, 3, 2);
        bdry_[vertices[3]] = q;

        link_[vertices[0]] = innerTet_[0];
        linkVertices_[vertices[0]] = vertices * NPerm<4>(0, 1, 3, 2);
    }

    QuadPrism::QuadPrism(const NTetrahedron *outerTet, int type,
            NTriangulation* insertInto) :
            Block(outerTet, 5, 4, insertInto) {
        innerTet_[4]->join(2, innerTet_[0], NPerm<4>());
        innerTet_[4]->join(3, innerTet_[1], NPerm<4>());
        innerTet_[4]->join(0, innerTet_[2], NPerm<4>());
        innerTet_[4]->join(1, innerTet_[3], NPerm<4>());
        ...
        q = new BdryQuad(this, vertices * NPerm<4>(2, 3, 1, 0));
        q->innerTet_[0] = innerTet_[2];
        q->innerTet_[1] = innerTet_[1];
        q->innerVertices_[0] = NPerm<4>(1, 0, 2, 3);
        q->innerVertices_[1] = NPerm<4>(2, 3, 1, 0);
        bdry_[vertices[0]] = q;

        q = new BdryQuad(this, vertices * NPerm<4>(3, 0, 2, 1));
        q->innerTet_[0] = innerTet_[3];
        q->innerTet_[1] = innerTet_[2];
        q->innerVertices_[0] = NPerm<4>(2, 1, 3, 0);
        q->innerVertices_[1] = NPerm<4>(3, 0, 2, 1);
        bdry_[vertices[1]] = q;

        q = new BdryQuad(this, vertices * NPerm<4>(0, 1, 3, 2));
        q->innerTet_[0] = innerTet_[0];
        q->innerTet_[1] = innerTet_[3];
        q->innerVertices_[0] = NPerm<4>(3, 2, 0, 1);
        q->innerVertices_[1] = NPerm<4>(0, 1, 3, 2);
        bdry_[vertices[2]] = q;

        q = new BdryQuad(this, vertices * NPerm<4>(1, 2, 0, 3));
        q->innerTet_[0] = innerTet_[1];
        q->innerTet_[1] = innerTet_[0];
        q->innerVertices_[0] = NPerm<4>(0, 3, 1, 2);
        q->innerVertices_[1] = NPerm<4>(1, 2, 0, 3);
        bdry_[vertices[3]] = q;
    }
    ...
    TruncHalfTet::TruncHalfTet(const NTetrahedron *outerTet, int type,
        ...
        innerTet_[1]->join(2, innerTet_[0], NPerm<4>());
        innerTet_[1]->join(1, innerTet_[2], NPerm<4>());
        innerTet_[1]->join(0, innerTet_[3], NPerm<4>());
        innerTet_[2]->join(0, innerTet_[4], NPerm<4>());
        innerTet_[3]->join(1, innerTet_[4], NPerm<4>());
        innerTet_[3]->join(3, innerTet_[5], NPerm<4>());
        innerTet_[5]->join(2, innerTet_[6], NPerm<4>());
        innerTet_[4]->join(2, innerTet_[7], NPerm<4>());
        ...
        h = new BdryHex(this, vertices * NPerm<4>(1, 3, 2, 0));
        h->innerTet_[0] = innerTet_[2];
        h->innerTet_[1] = innerTet_[7];
        h->innerTet_[2] = innerTet_[5];
        h->innerTet_[3] = innerTet_[4];
        h->innerVertices_[0] = NPerm<4>(2, 0, 1, 3);
        h->innerVertices_[1] = NPerm<4>(1, 2, 0, 3);
        h->innerVertices_[2] = NPerm<4>(0, 3, 2, 1);
        h->innerVertices_[3] = NPerm<4>(0, 2, 1, 3);
        bdry_[vertices[0]] = h;

        h = new BdryHex(this, vertices * NPerm<4>(0, 3, 2, 1));
        h->innerTet_[0] = innerTet_[0];
        h->innerTet_[1] = innerTet_[7];
        h->innerTet_[2] = innerTet_[6];
        h->innerTet_[3] = innerTet_[3];
        h->innerVertices_[0] = NPerm<4>(1, 2, 3, 0);
        h->innerVertices_[1] = NPerm<4>(3, 2, 0, 1);
        h->innerVertices_[2] = NPerm<4>(0, 2, 1, 3);
        h->innerVertices_[3] = NPerm<4>(0, 1, 3, 2);
        bdry_[vertices[1]] = h;

        q = new BdryQuad(this, vertices * NPerm<4>(3, 1, 0, 2));
        q->innerTet_[0] = innerTet_[2];
        q->innerTet_[1] = innerTet_[0];
        q->innerVertices_[0] = NPerm<4>(3, 1, 0, 2);
        q->innerVertices_[1] = NPerm<4>(0, 2, 3, 1);
        bdry_[vertices[2]] = q;

        q = new BdryQuad(this, vertices * NPerm<4>(2, 0, 1, 3));
        q->innerTet_[0] = innerTet_[6];
        q->innerTet_[1] = innerTet_[5];
        q->innerVertices_[0] = NPerm<4>(3, 2, 1, 0);
        q->innerVertices_[1] = NPerm<4>(1, 2, 3, 0);
        bdry_[vertices[3]] = q;

        link_[vertices[2]] = innerTet_[6];
        linkVertices_[vertices[2]] = vertices * NPerm<4>(3, 2, 0, 1);

        link_[vertices[3]] = innerTet_[7];
        linkVertices_[vertices[3]] = vertices * NPerm<4>(3, 1, 2, 0);
    }

    TruncTet::TruncTet(const NTetrahedron *outerTet, NTriangulation* insertInto) :
            Block(outerTet, 11, 16, insertInto) {
        innerTet_[0]->join(2, innerTet_[4], NPerm<4>());
        innerTet_[1]->join(3, innerTet_[7], NPerm<4>());
        innerTet_[2]->join(0, innerTet_[6], NPerm<4>());
        innerTet_[3]->join(1, innerTet_[9], NPerm<4>());
        innerTet_[5]->join(3, innerTet_[4], NPerm<4>());
        innerTet_[5]->join(1, innerTet_[6], NPerm<4>());
        innerTet_[8]->join(0, innerTet_[7], NPerm<4>());
        innerTet_[8]->join(2, innerTet_[9], NPerm<4>());
        innerTet_[4]->join(1, innerTet_[10], NPerm<4>());
        innerTet_[6]->join(3, innerTet_[10], NPerm<4>());
        innerTet_[7]->join(2, innerTet_[10], NPerm<4>());
        innerTet_[9]->join(0, innerTet_[10], NPerm<4>());

        BdryHex* h;

        h = new BdryHex(this, NPerm<4>(2, 1, 3, 0));
        h->innerTet_[0] = innerTet_[2];
        ...
        h->innerVertices_[0] = NPerm<4>(2, 0, 1, 3);
        h->innerVertices_[1] = NPerm<4>(1, 2, 0, 3);
        h->innerVertices_[2] = NPerm<4>(0, 1, 2, 3);
        h->innerVertices_[3] = NPerm<4>(0, 2, 1, 3);
        bdry_[0] = h;

        h = new BdryHex(this, NPerm<4>(3, 2, 0, 1));
        h->innerTet_[0] = innerTet_[3];
        h->innerTet_[1] = innerTet_[5];
        h->innerTet_[2] = innerTet_[0];
        h->innerTet_[3] = innerTet_[4];
        h->innerVertices_[0] = NPerm<4>(3, 1, 2, 0);
        h->innerVertices_[1] = NPerm<4>(2, 3, 1, 0);
        h->innerVertices_[2] = NPerm<4>(1, 2, 3, 0);
        h->innerVertices_[3] = NPerm<4>(1, 3, 2, 0);
        bdry_[1] = h;

        h = new BdryHex(this, NPerm<4>(0, 3, 1, 2));
        h->innerTet_[0] = innerTet_[0];
        h->innerTet_[1] = innerTet_[8];
        h->innerTet_[2] = innerTet_[1];
        h->innerTet_[3] = innerTet_[7];
        h->innerVertices_[0] = NPerm<4>(0, 2, 3, 1);
        h->innerVertices_[1] = NPerm<4>(3, 0, 2, 1);
        h->innerVertices_[2] = NPerm<4>(2, 3, 0, 1);
        h->innerVertices_[3] = NPerm<4>(2, 0, 3, 1);
        bdry_[2] = h;

        h = new BdryHex(this, NPerm<4>(1, 0, 2, 3));
        h->innerTet_[0] = innerTet_[1];
        h->innerTet_[1] = innerTet_[5];
        h->innerTet_[2] = innerTet_[2];
        h->innerTet_[3] = innerTet_[6];
        h->innerVertices_[0] = NPerm<4>(1, 3, 0, 2);
        h->innerVertices_[1] = NPerm<4>(0, 1, 3, 2);
        h->innerVertices_[2] = NPerm<4>(3, 0, 1, 2);
        h->innerVertices_[3] = NPerm<4>(3, 1, 0, 2);
        bdry_[3] = h;

        link_[0] = innerTet_[0];
        linkVertices_[0] = NPerm<4>(1, 2, 3, 0);

        link_[1] = innerTet_[1];
        linkVertices_[1] = NPerm<4>(1, 2, 3, 0);

        link_[2] = innerTet_[2];
        linkVertices_[2] = NPerm<4>(1, 2, 3, 0);

        link_[3] = innerTet_[3];
        linkVertices_[3] = NPerm<4>(1, 2, 3, 0);
    }
    ...
    inline Bdry::Bdry(Block* block, NPerm<4> outerVertices) :
    ...
    void BdryQuad::join(Bdry* other) {
        ...
        NPerm<4> destMap = block_->outerTet()->
    ...
    inline BdryQuad::BdryQuad(Block* block, NPerm<4> outerVertices) :
    ...
    void BdryQuad::reflect() {
        ...
        layering->join(0, innerTet_[1],
            innerVertices_[1] * NPerm<4>(3, 2, 1, 0));
        layering->join(2, innerTet_[0],
            innerVertices_[0] * NPerm<4>(1, 0, 3, 2));

        innerTet_[0] = innerTet_[1] = layering;
        innerVertices_[0] = NPerm<4>();
        innerVertices_[1] = NPerm<4>(2, 3, 0, 1);

        outerVertices_ = outerVertices_ * NPerm<4>(1, 2);
    }
    ...
    inline BdryHex::BdryHex(Block* block, NPerm<4> outerVertices) :
            Bdry(block, outerVertices) {
    }
    ...
    void BdryHex::reflect() {
        ...
        layering0->join(1, innerTet_[3], innerVertices_[3] * NPerm<4>(1, 3));
        layering0->join(2, innerTet_[2], innerVertices_[2] * NPerm<4>(2, 3));
        layering1->join(3, layering0, NPerm<4>());
        layering1->join(1, innerTet_[1],
            innerVertices_[1] * NPerm<4>(2, 3, 0, 1));
        layering2->join(0, layering0, NPerm<4>());
        layering2->join(1, innerTet_[0],
            innerVertices_[0] * NPerm<4>(1, 3, 2, 0));
        layering3->join(0, layering1, NPerm<4>());
        layering3->join(3, layering2, NPerm<4>());
        ...
        innerVertices_[0] = NPerm<4>(0, 3, 1, 2);
        innerVertices_[1] = NPerm<4>(1, 0, 3, 2);
        innerVertices_[2] = NPerm<4>(3, 2, 0, 1);
        innerVertices_[3] = NPerm<4>(3, 0, 1, 2);
    ...
    void BdryHex::rotate() {
        ...
        NPerm<4> p = innerVertices_[0];
        innerVertices_[0] = innerVertices_[1];
        innerVertices_[1] = innerVertices_[2];
        innerVertices_[2] = p;
        innerVertices_[3] = innerVertices_[3] * NPerm<4>(1, 2, 0, 3);

        outerVertices_ = outerVertices_ * NPerm<4>(1, 2, 0, 3);
    }
        ...
        if (quadTypes[whichTet] == -1) {
            ...
            for (face = 0; face < 4; face++) {
                ...
                while (adj && (adjQuads >= 0)) {
                    swap = NPerm<4>(adjFace, quadPartner[adjQuads][adjFace]);
