...
void RetriangulateParams<Link>::propagateFrom(const std::string& sig,
    ...
    using Options = typename regina::detail::PropagationOptions<Link>;
    ...
    if (t.size() == 0) {
        ...
        if constexpr (Retriangulator::options == Options::ClassicalAndVirtual) {
            if (maxSize > 1) {
...
template bool detail::retriangulateInternal<Link, true,
        ...
        PropagationOptions<Link>::ClassicalOnly>(
...
template bool detail::retriangulateInternal<Link, true,
        detail::RetriangulateDefault,
        PropagationOptions<Link>::ClassicalAndVirtual>(
    const Link&, bool, int, int, ProgressTrackerOpen*,
    regina::detail::RetriangulateActionFunc<Link, true>&&);
