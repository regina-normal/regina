...
std::pair<size_t, Arrow::DiagramSequence> Link::resolutionCuspedLoops(
        uint64_t mask) const {
    ...
    // We will count the number of cusp pairs (i.e., pairs of nodal arrows) in
    // each loop that we find.  Note that 2n strands will give ≤ n cusp pairs.
    //
            ...
            if (! found[pos + (dirInit ? n : 0)]) {
                ...
                do {
                    ...
                    if (s.crossing()->sign() > 0) {
                        // Positive crossing:
                        if ((mask & bit) == 0) {
                            // Turn in a manner consistent with the arrows.
                            nodalArrow = 0;
                            if (dir == 1) {
                            ...
                            // Turn in a manner inconsistent with the arrows.
                    ...
                    if (nodalArrow) {
                        if (nodalArrowCount) {
                            if (nodalArrow == lastNodalArrow) {
                                // This nodal arrow cancels the previous one.
                                --nodalArrowCount;
                            } else {
                ...
                if (nodalArrowCount) {
                    // It should always be true that nodalArrowCount is even.
                    if (nodalArrowCount & 1)
                        throw ImpossibleScenario("A resolution gives a loop "
                            "with an odd number of nodal arrows");

                    nodalArrowCount >>= 1; // convert nodal arrows to cusp pairs
...
Arrow Link::arrowNaive(ProgressTracker* tracker) const {
    ...
    for (uint64_t mask = 0; mask != maskEnd; ++mask) {
        ...
        auto [ loops, diagramSequence ] = resolutionCuspedLoops(mask);
        loops += trivialLoops;
        if (loops > maxLoops)
            maxLoops = loops;
