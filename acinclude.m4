...
dnl        TARGET, REQUIRED-BY)
dnl
dnl    Checks for the given optional library.
dnl    Issues a warning and adds <TARGET> to $DO_NOT_COMPILE if the
dnl    library is missing.
...
AC_DEFUN(REGINA_LIB_OPTIONAL, [
  AC_CHECK_LIB([$1], [$2], [
    ...
    DO_NOT_COMPILE="$DO_NOT_COMPILE $4"
  ])
])

dnl
dnl    REGINA_HEADER_OPTIONAL(HEADER, TARGET, REQUIRED-BY)
dnl
dnl    Checks for the given optional header.
dnl    Issues a warning and adds <TARGET> to $DO_NOT_COMPILE if the
dnl    header is missing.
dnl
