...
/**
 * Combines the functions of tricensus-mpi and normal-mpi.
 *
 * Command-line options are the same as for tricensus-mpi; however,
 * instead of saving the triangulations, this utility merely enumerates
 * normal surfaces (in both standard and quad coordinates) and writes
 * the resulting counts to *.sdat and *.qdat.
 *
...
void slaveFoundGluingPerms(const regina::GluingPermSearcher<3>* perms, void*) {
    if (perms) {
        ...
        if (ok) {
            ...
            // Enumerate normal surfaces!  Use unique filenames for output.
            if (! serr) {
                if (! sout.is_open()) {
                    std::string outFile;
                    slaveMakeTaskFilename(outFile, ".sdat");
            ...
            if (! qerr) {
                if (! qout.is_open()) {
                    std::string outFile;
                    slaveMakeTaskFilename(outFile, ".qdat");

                    qout.open(outFile.c_str());
                    if (! qout)
                        qerr = true;
                }
                if (! qerr) {
...
void slaveProcessPartialSearch() {
    ...
    if (nSolns > 0) {
        // Close the output files and return our results.
        if (sout.is_open())
            sout.close();
        if (qout.is_open())
            qout.close();
        ...
        if (serr)
            slaveBail("Could not open output file for standard coords.");
        else if (qerr)
            slaveBail("Could not open output file for quad coords.");
        else
