...
int dimension = -1; // dimension 0 indicates knot/link signatures
int sigGen = 2;

void help() {
    std::cerr <<
R"help(Usage: sigconvert <isosig> ...
  -d, --dim=<dimension>       Input is an isomorphism signature for the given
                              dimension of triangulation (default = 3)
  -k, --knot                  Input is a knot/link signature
  -2, --dim2                  Alias for --dim=2
  -3, --dim3                  Alias for --dim=3
  -4, --dim4                  Alias for --dim=4
...
int main(int argc, char* argv[]) {
            ...
            case 'd':
                if (dimension >= 0) {
                    std::cerr << "You cannot pass more than one of "
                        "--dim, --knot, --dim2, --dim3, or --dim4.\n\n";
                    help();
                    return 1;
                }
                dimension = strtol(optarg, &endptr, 10);
                if (*endptr != 0 || dimension < 2 ||
                        dimension > regina::maxDim()) {
                    std::cerr << "The dimension must be an integer between "
                        "2 and " << regina::maxDim() << " inclusive.\n\n";
                    help();
                    return 1;
                }
                break;
    ...
    if (optind < argc) {
        if (dimension == 0) {
            ...
            regina::select_constexpr<2, regina::maxDim() + 1, void>(
                    dimension, [=, &success](auto dim) {
                for (int i = optind; i < argc; ++i)
        ...
        if (dimension == 0) {
            ...
            while (! std::cin.eof()) {
                regina::select_constexpr<2, regina::maxDim() + 1, void>(
                        dimension, [=, &sig, &success](auto dim) {
