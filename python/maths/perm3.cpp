...
void addPerm3(pybind11::module_& m) {
    ...
    auto c = pybind11::class_<Perm<3>>(m, "Perm3", rdoc_scope)
        ...
        .def(pybind11::init<int, int, int>(), rdoc::Perm_3)
        .def(pybind11::init<const std::array<int, 3>&>(), rdoc::Perm_4)
        .def(pybind11::init<int, int, int, int, int, int>(), rdoc::Perm_5)
        .def(pybind11::init<const Perm<3>&>(), rdoc::Perm_6)
