...
// Docstring regina::python::doc::Component4::__class
...
lower-dimensional faces (i.e., vertices, edges, triangles and
tetrahedra).
...
Python:
    This class is available to Python users under the name Component4.)doc";

// Docstring regina::python::doc::Component4::countBoundaryTetrahedra
...
// Docstring regina::python::doc::Component4::countFaces
...
4. This is _not_ the case for the routines face() and faces(), which
give access to individual faces (the reason relates to the fact that
pentachora are built manually, whereas lower-dimensional faces are
deduced properties).

Exception ``InvalidArgument``:
    The face dimension *subdim* is outside the supported range (i.e.,
    negative or greater than 4).
...
// Docstring regina::python::doc::Component4::face
...
store a pointer to any class ``Face<4, ...>``. This means you cannot
...
appropriate ``Face<4, subdim>`` object from *v*. However, once you
...
``Face<4, subdim>`` object of the appropriate face dimension that you
can use immediately.
...
// Docstring regina::python::doc::Component4::faces
...
// Docstring regina::python::doc::Component4::hasBoundaryTetrahedra
...
// Docstring regina::python::doc::Component4::isClosed
...
Note that ideal components are not closed. Likewise, components with
invalid vertices are not closed. See Vertex<4>::isBoundary() for
details.

Returns:
    ``True`` if and only if this component is closed.)doc";

// Docstring regina::python::doc::Component4::isIdeal
static constexpr const char isIdeal[] =
R"doc(Determines if this component is ideal. This is the case if and only if
it contains an ideal vertex as described by Vertex<4>::isIdeal().
