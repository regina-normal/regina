...
This access is read-only: both the list itself and the list elements
are read-only. (Of course, if the list elements are non-const pointers
then this means that the *pointers* cannot be reassigned to point to
different objects, but the objects they *point* to can still be
modified.)
