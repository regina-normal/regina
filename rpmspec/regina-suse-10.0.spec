...
Patch0: regina-mpich_ch-p4.patch
Patch1: regina-nopython-suse10.0.patch
...
Note that Python scripting has been disabled in this package because
SuSE 10.0 ships with a broken Boost.Python installation.  SuSE plans
to fix these problems in SuSE 10.1, at which point Regina scripting
for SuSE will be re-enabled.
...
./configure --disable-python --disable-debug --includedir=%{_includedir} --mandir=%{_mandir} MPICXX=/opt/mpich/ch-p4/bin/mpicxx
make
make check
