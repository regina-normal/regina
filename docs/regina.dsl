...
; === Media objects ===
(define preferred-mediaobject-extensions ;; this magic allows to use different
...
(define %head-after-factor% 0.2)        ;; not much whitespace after
                                        ;; orderedlist head
(define ($paragraph$)                   ;; more whitespace after paragraph
  (make paragraph                       ;; than before
...
(define %html-prefix% "")           ;; prefix for all filenames generated
                                    ;;   (except root)
(define %use-id-as-filename% #t)    ;; uses ID value, if present, as filename
                                    ;;   otherwise a code is used to indicate
                                    ;;   level of chunk, and general element
                                    ;;   number (nth element in the document)
...
; === Media objects ===
(define preferred-mediaobject-extensions    ;; this magic allows to use
  (list "png" "jpg" "jpeg"))                ;; different graphical formats
(define acceptable-mediaobject-extensions   ;; for printing and putting online
...
(define ($generate-chapter-toc$) #f)    ;; never generate chapter TOC in books
